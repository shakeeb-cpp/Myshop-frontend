              ...
              return (
                <div key={item.attributes.slug} className="xl:w-1/4 md:w-1/3 p-2  m-auto">
                  ...
                  <div className='border-4 border-gray-100 bg-white cursor-pointer p-5 m-auto  hover:shadow-lg'>
                    <img className=" m-auto transition ease-in-out delay-75 w-64 hover:-translate-y-1 hover:scale-110 duration-500 rounded object-contain object-center h-64 " src={`http://127.0.0.1:1337${item.attributes.image.data.attributes.url}`} alt="content" />
                  </div>
...
export async function getServerSideProps(context) {
  ...
  let a = await fetch("http://127.0.0.1:1337/api/products?populate=*", { headers: headers });
