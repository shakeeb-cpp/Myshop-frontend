                ...
                return (
                  <div key={item.attributes.slug} className="lg:w-1/4 md:w-1/3 w-1/2  md:p-4 p-0 cursor-pointer mx-auto">
                    <div className="flex relative">
                      <img alt="gallery" className="absolute border p-2 inset-0 w-72 h-full object-contain object-center" src={`http://127.0.0.1:1337${item.attributes.image.data.attributes.url}`} />
              ...
              {props.products2.data.map((item) => {
                return (
                  <div key={item.attributes.slug} className="lg:w-1/4 sm:w-1/2 p-4 cursor-pointer mx-auto">
                    <div className="flex relative">
                      <img alt="gallery" className="p-6 border-b-2 border-rose-600  inset-0 w-96 h-96 object-contain object-center" src={`http://127.0.0.1:1337${item.attributes.image.data.attributes.url}`} />
...
export async function getServerSideProps(context) {
  ...
  let a = await fetch("http://127.0.0.1:1337/api/top-products?filters[image][id][$lte][0]=4&populate[0]=image", { headers: headers });
  let products = await a.json();

  let a2 = await fetch("http://127.0.0.1:1337/api/top-products?filters[image][id][$gte][0]=5&populate[0]=image", { headers: headers });
